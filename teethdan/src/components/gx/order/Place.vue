...
//        window.location.href = "#/common/errorPage"
      ...
      checkDisc: function(disc, id) { // 选择地区
        ...
        // 跳转到医院列表页面
        setTimeout(function() {sessionStorage.setItem('back_ticketURI','0');window.location.href = "#/gx/order/checkStation"}, 100)
      },
      // 索引当前跳转设置
      curIndex: function() {
        if(this.curid != "") {
...
//      setTimeout(()=>{$(".cityListArea").toggleClass("city_top");},200)
      ...
      findCity: function() {
        ...
        var place = JSON.parse(sessionStorage.getItem('orderInfo'));
        // 跳转医院列表页面
        if(_this.localFlag == false && place.cityId){ sessionStorage.setItem('back_ticketURI','0');window.location.href = "#/gx/order/checkStation" }
      },
//    滑动结束
      touchEnd(){
       	 var opcityNum = 1;
