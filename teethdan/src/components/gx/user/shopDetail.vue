...
<script>
	import $ from 'jquery';
	import {Toast,Swipe,SwipeItem,Indicator,MessageBox,Loadmore,InfiniteScroll} from 'mint-ui';
    	...
    	if(pic&&pic!='none'){//关闭弹出层 &&diswrap=='block'
    		...
    		var toMap="";
		  	...
		  	if(toMap=='0'){//此情况为从地图返回
		  		this.backTime=mapRouter.backTime;
		  		sessionStorage.removeItem('mapRouter')
		  		history.go(this.backTime);
		  	}else{
		  		next();
		  	}
			}
...
//	        	window.location.replace("#/common/errorPage");//跳转至错误页面
	      ...
	      goOrder:function(){
	      	sessionStorage.removeItem('mapRouter');
		      window.location.href = "#/common/enterRouter?page="+encodeURIComponent("gx/order/toOrder")
		  },
