      ...
      <div class="orderInfo">
        ...
        <div class='pro_content'>
          ...
          <ul class='pro_txt'>
            ...
            <li v-show="orderInfo.source=='1'"><span class='s_content'>口腔全景摄片1张 </span><span class='s_content'>超声洁牙抛光1次</span></li>
...
//            WeixinJSBridge.call('closeWindow');
    ...
    beforeCreate() {
      document.title = '预约';sessionStorage.setItem('isOrder',false);sessionStorage.setItem('back_ticketURI','1');
    },
    created() {tkWX.init({});},
    mounted() {
...
//            orderMsg.orderState = "1";  
            ...
            if(omstate=="4" ||omstate=="0"){// 已经预约
              ...
              window.location.href = "#/user/orderDetail?orderId=" +that.orderInfo.orderId + "&policyNo=" + that.orderInfo.policyNo + '&channelType=' + that.orderInfo.channelType + '&cpnTicketNo=' + that.orderInfo.cpnTicketNo
            }else if(omstate=="3"||omstate=="1"){// 已完成
              sessionStorage.setItem('back_ticketURI','0')
              window.location.href='#/user/evalFill?orderId='+that.orderInfo.orderId+'&policyNo='+that.orderInfo.policyNo+'&channelType='+that.orderInfo.channelType+'&cpnTicketNo='+that.orderInfo.cpnTicketNo
        ...
        if(_this.isCheck == true && _this.phoneflag == true && _this.bookDateTime != "" && _this.orderFlag == 1) {
          ...
          response.then(function(res) {
            ...
            if(res.data.status == '1') {
              ...
              setTimeout(function() {
                sessionStorage.setItem('back_ticketURI','0');
                window.location.href = "#/common/enterRouter?page="+encodeURIComponent("user/orderDetail&orderId=") + res.data.orderId + "&policyNo=" + _this.orderInfo.policyNo + '&channelType=' + _this.orderInfo.channelType + '&cpnTicketNo=' + _this.orderInfo.cpnTicketNo+'&wesurePolicyNo='+ _this.orderInfo.wesurePolicyNo;
              }, 100)
