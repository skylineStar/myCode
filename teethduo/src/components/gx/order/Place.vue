    ...
    beforeRouteLeave:(to, from, next) => {
      if(sessionStorage.getItem('back_ticketURI')){//返回卡券标记
          ...
          if(back_ticketURI=='1'){//返回卡券
              ...
              window.location.href='gxOrder.html#/gx/order/toOrder';
          }else{
              next();
          }
      }else{
...
//        window.location.href = "#/common/errorPage"
      ...
      checkCity: function(item, cityId, indexId) {
        ...
        if(item.field == "") {if(config_param.isLog)console.log('" "--为泰康人寿') }
...
//        window.location.href = "#/common/errorPage"
      ...
      checkDisc: function(disc, id) { // 选择地区
        ...
        // 跳转到医院列表页面
        setTimeout(function() {sessionStorage.setItem('back_ticketURI','0');
      ...
      toList: function() {
        if(this.checkCityName != "") {
          ...
          if(place.cityId) {
            ...
            setTimeout(()=>{ sessionStorage.setItem('back_ticketURI','0');
//          window.location.href = "gxOrder.html#/gx/order/checkStation"
            this.$router.push({path: '/gx/order/checkStation'});
            }, 100)
          }else{
      ...
      findCity: function() {
        ...
        var place = JSON.parse(sessionStorage.getItem('orderInfo'));
        // 跳转医院列表页面
        if(_this.localFlag == false && place.cityId){ sessionStorage.setItem('back_ticketURI','0');
//      window.location.href = "gxOrder.html#/gx/order/checkStation" 
        this.$router.push({path: '/gx/order/checkStation'});
        }
      },
