...
<template>
	...
	<div class="orderDetail">
    	...
    	<!--<div @click='closepop' class="gxpop_modal" v-if='isordert' style="z-index: 1001;"></div>-->
...
//						that.source = '7'
						...
						if(omstate=="4"){//已确认预约
							...
							sessionStorage.setItem('back_ticketURI','0');
							window.location.href='gxUser.html#/gx/user/evalFill?orderId='+that.orderId+'&policyNo='+that.policyNo+'&channelType='+channel+'&cpnTicketNo='+cpnTicketNo+'&userId='+that.userId
						}else if(omstate=="5"){//已爽约
							sessionStorage.setItem('back_ticketURI','0');
							window.location.href='gxUser.html#/gx/user/unorder?orderId='+that.orderId
						}else if(omstate=="2"){//已取消预约
							sessionStorage.setItem('back_ticketURI','0');
							window.location.href='gxOrder.html#/gx/order/toOrder?orderId='+that.orderId+'&policyNo='+that.policyNo+'&channelType='+channel+'&cpnTicketNo='+cpnTicketNo+'&wesurePolicyNo='+wesurePolicyNo
						}
						...
						parResult.then(function(res){
							...
							if(data.status=="1"){
								...
								setTimeout(function(){
									...
									window.location.href='#/common/enterRouter?page=gxOrder.html#/gx/order/toOrder&orderId='+orderIds+'&policyNo='+that.policyNo+'&channelType='+channelType+'&cpnTicketNo='+cpnTicketNo+'&wesurePolicyNo='+wesurePolicyNo},1000)
							}else{
								Toast (data.message);
							}
						})
