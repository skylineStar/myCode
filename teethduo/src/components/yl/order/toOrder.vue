...
//        window.location.replace("#/common/errorPage");
        ...
        parResult.then(function(res){
          ...
          if(data.status=="1"){
            ...
            if(omstate=="4" ||omstate=="0"){// 已经预约
              sessionStorage.setItem('back_ticketURI','0')
              window.location.href = "gxUser.html#/yl/user/orderDetail?orderId=" +that.orderInfo.orderId + "&policyNo=" + that.orderInfo.policyNo + '&channelType=' + that.orderInfo.channelType + '&cpnTicketNo=' + that.orderInfo.cpnTicketNo
              console.log(that.orderInfo.orderId)
            }else if(omstate=="3"||omstate=="1"){// 已完成
              sessionStorage.setItem('back_ticketURI','0')
              window.location.href='gxUser.html#/yl/user/evalFill?orderId='+that.orderInfo.orderId+'&policyNo='+that.orderInfo.policyNo+'&channelType='+that.orderInfo.channelType+'&cpnTicketNo='+that.orderInfo.cpnTicketNo
            }else if(omstate=="5"){// 爽约
              sessionStorage.setItem('back_ticketURI','0')
              window.location.href='gxUser.html#/yl/user/unorder?orderId='+that.orderInfo.orderId
      ...
      toServeContent:function(){
        sessionStorage.setItem('back_ticketURI','0')
        this.$router.push({path: '/yl/order/serveContent'});
      },
       // 服务知情同意书
      toServeNotice:function(){
        sessionStorage.setItem('back_ticketURI','0')
        this.$router.push({path: '/yl/order/agreeServe'});
      },
      // 去选择医院信息
      toCheckHospital: function() {
        ...
        if(this.orderInfo.cityId && this.onLoadFlag==false) {
          sessionStorage.setItem('back_ticketURI','0')
          this.$router.push({path: '/yl/order/checkStation'});
        } else {
          if(this.orderInfo.channelType) {
            sessionStorage.setItem('back_ticketURI','0')
            this.$router.push({path: '/yl/order/Place'});
          } 
        }
      },
      // 去选择预约时间
      toCheckTime: function() {
        if(this.orderInfo.hospitalId) {
          sessionStorage.setItem('back_ticketURI','0')
          window.location.href = "gxOrder.html#/yl/order/checkTime"
        } else {
        ...
        if(_this.isCheck == true && _this.phoneflag == true && _this.bookDateTime != "" && _this.orderFlag == 1) {
          ...
          response.then(function(res) {
            ...
            if(res.data.status == '1') {
              ...
              setTimeout(function() {
                sessionStorage.setItem('back_ticketURI','0')
                window.location.href = "#/common/enterRouter?page=gxUser.html#/yl/user/orderDetail&orderId="+res.data.orderId+ "&policyNo="+ _this.orderInfo.policyNo+ '&channelType=' + _this.orderInfo.channelType + '&cpnTicketNo=' + _this.orderInfo.cpnTicketNo+'&wesurePolicyNo='+ _this.orderInfo.wesurePolicyNo;
              }, 100)
            ...
            _this.showbtn();
            if(config_param.isLog)console.log(err);
//          window.location.replace("#/yl/common/errorPage");
          });
        }
