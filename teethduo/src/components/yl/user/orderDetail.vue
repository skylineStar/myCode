...
//				    window.location.href='#/common/toMapEmpty';
				    this.$router.push({path: '/common/toMapEmpty'});
			    }
			},
			timeOut:function(){
				var that = this;
...
//						that.source = '7'
						...
						if(omstate=="4"){//已确认预约
							...
							sessionStorage.setItem('back_ticketURI','0');
							window.location.href='ylUser.html#/yl/user/evalFill?orderId='+that.orderId+'&policyNo='+that.policyNo+'&channelType='+channel+'&cpnTicketNo='+cpnTicketNo+'&userId='+that.userId
						}else if(omstate=="5"){//已爽约
							sessionStorage.setItem('back_ticketURI','0');
							window.location.href='ylUser.html#/yl/user/unorder?orderId='+that.orderId
						}else if(omstate=="2"){//已取消预约
							sessionStorage.setItem('back_ticketURI','0');
							window.location.href='ylUser.html#/yl/order/toOrder?orderId='+that.orderId+'&policyNo='+that.policyNo+'&channelType='+channel+'&cpnTicketNo='+cpnTicketNo+'&wesurePolicyNo='+wesurePolicyNo
						}
						...
						parResult.then(function(res){
							...
							if(data.status=="1"){
								...
								setTimeout(function(){
									...
									if(back_ticketURI=='1')sessionStorage.setItem('back_ticketURI','0');
									window.location.href='#/common/enterRouter?page=ylOrder.html#/yl/order/toOrder&orderId='+orderIds+'&policyNo='+that.policyNo+'&channelType='+channelType+'&cpnTicketNo='+cpnTicketNo+'&wesurePolicyNo='+wesurePolicyNo},1000)
							}else{
								Toast (data.message);
							}
						})
